...
The project uses Oxlint and Oxfmt with a `complete-check` script for full quality validation. We need to implement git hooks that automatically enforce code quality at commit and push time while maintaining developer productivity.
...
2. Configure lint-staged for TypeScript/React files with Oxc tooling integration
...
| **Husky + Ox staged commands** | Fast lint/format split, clear responsibilities                       | Two commands to maintain                                | ❌ More wiring than lint-staged |
