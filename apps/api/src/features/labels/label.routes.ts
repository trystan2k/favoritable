...
import { LabelService } from "./label.services.js";
...
export class LabelRoutes {
  ...
  private setupRoutes() {
    ...
    // Bookmarks - Delete
    this.labelRoutes.post('/batch-delete', zCustomValidator('json', deleteLabelssSchema), async (c) => {
      const data = c.req.valid('json');
    ...
    // Label - Update
    this.labelRoutes.put('/:id', zCustomValidator('param', labelIdParamSchema), zCustomValidator('json', updateLabelSchema.partial()), async (c) => {
      const data = c.req.valid('json');
      const { id } = c.req.valid('param');
      data.id = id;
