...
import {
  ...
  deleteBookmarksSchema,
  getBookmarksQueryParamsSchema,
  importFromHTMLFileQueryParamsSchema,
  importOmnivoreBookmarksSchema,
  type UpdateBookmarkModel,
  updateBookmarkSchema,
} from './bookmark.models.js';
import type { BookmarkService } from './bookmark.services.js';
...
export class BookmarkRoutes {
  ...
  private setupRoutes() {
    ...
    // Bookmark - Update
    this.bookmarkRoutes.patch(
      '/:id',
      zCustomValidator('json', updateBookmarkSchema),
      zCustomValidator('param', bookmarkIdParamSchema),
      async (c) => {
        const data = c.req.valid('json');
        const { id } = c.req.valid('param');
    ...
    importRoutes.post(
      '/omnivore',
      zCustomValidator('json', importOmnivoreBookmarksSchema.array()),
      async (c) => {
        const data = c.req.valid('json');
    ...
    importRoutes.post(
      '/html',
      zCustomValidator('query', importFromHTMLFileQueryParamsSchema),
      async (c) => {
