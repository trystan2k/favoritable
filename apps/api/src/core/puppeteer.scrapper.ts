...
const getTitle = async (page: Page) => {
  ...
  if (!title) {
    title = await page
      .$eval('meta[property="title"]', (el) => el.getAttribute('content'))
      .catch(() => null);
  }
...
const getAuthor = async (page: Page) => {
  ...
  if (!author) {
    author = await page
      .$eval('[data-testid="User-Name"] > div', (el) => el.textContent)
      .catch(() => null);
  }
...
const getPublishedAt = async (page: Page) => {
  ...
  if (!publishedAt) {
    publishedAt = await page
      .$eval('[datetime]', (el) => el.getAttribute('datetime'))
      .catch(() => null);
  }
...
const getThumbnail = async (page: Page) => {
  let thumbnail = await page
    .$eval('meta[property="og:image"]', (el) => el.getAttribute('content'))
    .catch(() => null);

  if (!thumbnail) {
    thumbnail = await page
      .$eval('meta[name="og:image"]', (el) => el.getAttribute('content'))
      .catch(() => null);
  }

  if (!thumbnail) {
    thumbnail = await page
      .$eval('meta[name="image"]', (el) => el.getAttribute('content'))
      .catch(() => null);
  }
  ...
  if (!thumbnail) {
    thumbnail = await page
      .$eval('article[data-author-name] > img', (el) => el.getAttribute('src'))
      .catch(() => null);
  }
